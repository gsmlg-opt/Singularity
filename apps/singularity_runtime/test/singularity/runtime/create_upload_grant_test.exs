defmodule Singularity.Runtime.CreateUploadGrantTest do
  ...
  test "creates a five-minute grant bound to the exact request" do
    ...
    assert command.resource_version_id == request.resource_version_id
    assert command.classification == :sensitive
  end
  ...
  test "enforces the server upload limit before scope even when attrs try to raise it" do
    runtime = %{
      assets: {Assets, self()},
      clock: fn -> DateTime.utc_now() end,
      id_generator: fn -> Ecto.UUID.generate() end,
