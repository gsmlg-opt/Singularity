defmodule Singularity.Runtime.Notes.History do
  @moduledoc "Reads cursor-paginated immutable note history."
  ...
  def run(runtime, %SessionContext{} = session, resource_id, params) when is_map(runtime) do
    with true <- valid_uuid?(resource_id),
         {:ok, page} <- page_params(params),
      ...
      call_adapter(adapters.operation_scope, :with_read_request, [
        ...
        requirement(session),
        fn repo ->
          call_adapter(adapters.note_repository, :history, [
            repo,
            session.vault_id,
            resource_id,
            page
  ...
  def run(_runtime, _session, _resource_id, _params), do: invalid()
